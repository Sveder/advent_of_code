...
def print_board(board):
    for line in board:
        ...
        for i, tile in enumerate(line):
            cur_row += str(tile.tile.name) + " " * 11

        cur_row += "\n"

        for i in range(10):
            for t in line:
                cur_row += t.arr_list[i] + " " * 5
...
for tile_obj in tile_dict.values():
    for can in tile_obj.possible_arr:
        if can.neighbours['down'] != None and can.neighbours['left'] != None and can.neighbours['right'] == None and can.neighbours['up'] == None:
            ...
            possible_corners.append(can)


possible_boards = {}
for i, corner in enumerate(possible_corners):
    square = []
    square_size = int(len(tile_dict) ** 0.5)
    ...
    for a in range(square_size):
        ...
        for b in range(square_size):
            row.append(cur_arr)
            if cur_arr:
                cur_arr = cur_arr.neighbours.get('left')

        square.append(row)
        if row_pointer:
            cur_arr = row_pointer.neighbours.get('down')
            row_pointer = cur_arr

    possible_boards[i] = square

# Create one array without the borders:
pictures = {}
for k, v in possible_boards.items():
    pic = ""
...
for pic in pictures.values():
    ...
    for i, row in enumerate(pic.split('\n')):
        for j, cell in enumerate(row):
            if cell == '.':
                new_pic += cell

            if 2 < i < len(row) - 3 and 2 < j < len(row):
    ...
    print('Monsters:', monsters, " count:", non_monster - monster_count * monsters)
