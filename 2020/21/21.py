...
allergies_to_ing_candidates = collections.defaultdict(list)
all_ing = []

for line in input:
    ing, allergies = line.split(" (contains ")
    ing = ing.split(' ')
    allergies = allergies[:-1].split(", ")
    all_ing += ing
    for a in allergies:
        allergies_to_ing_candidates[a].append(ing)


food_to_allergy = {}
uniq_all_ing = list(set(all_ing))
...
while True:
    ...
    for all, ing_lists in allergies_to_ing_candidates.items():
        if len(ing_lists) > 1:
            inter = set(ing_lists[0])

            for i in ing_lists[1:]:
                inter = inter.intersection(i)

            potential = []

            for i in inter:
                if i not in food_to_allergy.keys():
                    potential.append(i)

            if len(potential) == 1:
                food_to_allergy[list(potential)[0]] = all
                found = True

        elif len(ing_lists) == 1:
            potential = list(set(ing_lists[0]) - set(food_to_allergy.keys()))

            print("In the second", ing_lists, potential)

            if len(potential) == 1 and potential[0] not in food_to_allergy.keys():
                food_to_allergy[list(potential)[0]] = all
                found = True
...
print("Food:", food_to_allergy)
pprint.pprint(set(all_ing) - set(food_to_allergy))
...
allergens_by_alphabet = list(food_to_allergy.values())
allergens_by_alphabet.sort()

allergy_to_food = {v: k for k,v in food_to_allergy.items()}

dangerous = []
for i in allergens_by_alphabet:
    dangerous.append(allergy_to_food[i])
