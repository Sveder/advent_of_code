...
rules, cases = input.split("\n\n")
rule_dict = {}
expanded_dict = {}


def expand(rule_number):
    global rule_dict, expanded_dict
    stmt = rule_dict[rule_number]

    if rule_number in expanded_dict.keys():
        return expanded_dict[rule_number]

    if stmt in ('"a"', '"b"'):
        expanded_dict[rule_number] = stmt[1]
        return [stmt[1]]

    allowed = [""]
    end_allowed = []
    new_allowed = [""]
    for sub_rule in stmt.split(" | "):
        for x, rule_part in enumerate(sub_rule.split(" ")):
            new_allowed_cache = list(new_allowed)

            # if rule_number == 2:
            #     import ipdb;ipdb.set_trace()

            int_rule_part = int(rule_part)
            expand(int_rule_part)

            new_values = expanded_dict[int_rule_part]

            for i in list(new_allowed):
                for j in new_values:
                    # print(i + j)
                    new_allowed.append(i + j)

            new_allowed = list(set(new_allowed) - set(new_allowed_cache))

        end_allowed += list(set(new_allowed))
        new_allowed = [""]

    # print("Expanding rule number:", rule_number, ' stmt ', stmt, ' allowed ', end_allowed)
    print("Expanding rule number:", rule_number, ' stmt ', stmt)
    expanded_dict[rule_number] = end_allowed


for rule in rules.split("\n"):
    rule_num, content = rule.split(": ")
    rule_dict[int(rule_num)] = content

# Patch:
rule_dict[8] = "42 | 42 8"
rule_dict[11] = "42 31 | 42 11 31"

print("Rule 0:", rule_dict[0])
...
for i in cases.split("\n"):
    ...
    if i == "abbbbbabbbaaaababbaabbbbabababbbabbbbbbabaaaa":
        import ipdb;ipdb.set_trace()

    for part in textwrap.wrap(i, len(ft[0])):
        if part in to and in_ft == True:
            to_count += 1
            in_ft = False
            continue

        if part in to and in_ft == False:
            to_count += 1
            continue

        if part in ft and in_ft == True:
            ft_count += 1
            continue

        if part in ft and in_ft == False:
            found = False
            break
    ...
    if in_ft == False and ft_count > to_count:
        results.append(i)
