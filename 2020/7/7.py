...
class Bag():
    def __init__(self, color, contents):
        ...
        # Contents is {color: count}

    def __str__(self):
        return "Bag of color %s, with content %s" % (self.color, self.contents)
    ...
    def parse_line(line):
        ...
        contents_part = line.split("contain ")[-1]
        contents = {}
        if "no other bags" not in contents_part:
            for content_desc in contents_part.split(", "):
                desc_parts = content_desc.split(" ")
                part_color = " ".join(desc_parts[1:3])
                amounts = int(desc_parts[0])
                contents[part_color] = amounts

        return Bag(color, contents)


def get_all_content_colors(bag, all_bags):
    if bag.contents == {}:
        return []

    result = list(bag.contents.keys())
    for color in bag.contents.keys():
        result += get_all_content_colors(all_bags[color], all_bags)
...
for bag_color, bag in bags.items():
    all_containing = get_all_content_colors(bag, bags)
    if "shiny gold" in all_containing:
        container_count += 1

print("Container count:", container_count)

def count_bags_in(bag, all_bags):
    if bag.contents == {}:
        return 1

    result = 1
    for color in bag.contents.keys():
        result += bag.contents[color] * count_bags_in(all_bags[color], all_bags)

    return result

print('Bag in shiny gold:', count_bags_in(bags["shiny gold"], bags))