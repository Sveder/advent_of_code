input = """LLLLLLLLLLL..LLLLLL.LLLLL.LLLLLLLLLLLLLLL.LLLLLLLLLLLLLLLLLLLLLLLL.LLLLLL.LL.LLLLLLLL..LLLLL
...
def count_neighbors(i, j, input):
    ...
    for d in directions:
        if 0 <= i + d[0] < len(input):
            if 0 <= j + d[1] < len(input[0]):
                if input[i + d[0]][j + d[1]] == "#":
                    count += 1

    return count

def count_neighbors_2(i, j, input):
    count = 0
    directions = [
        (-1, -1),
        (-1, 0),
    ...
    for d in directions:
        cur_iter = 1
        while (0 <= i + d[0] * cur_iter < len(input)) and (0 <= j + d[1] * cur_iter < len(input[0])):
            if input[i + d[0] * cur_iter][j + d[1] * cur_iter] in ("#", "L"):
                if input[i + d[0] * cur_iter][j + d[1] * cur_iter] == "#":
                    count += 1
                break
